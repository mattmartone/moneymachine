...
async function scoreRace(raceId) {
  ...
  // Longshot Audit: picks at 20-1+ need extra validation
  if (pickOdds && pickOdds >= 20 && conviction === 'HIGH') {
    const daysSince = winPick?.days_since_last || 0;
    const beyerGap = (winPick?.best_beyer || 0) - (winPick?.last_beyer || 0);
    const hasStaleBeyer = daysSince >= 45 && beyerGap >= 10;
    const tooFewSignals = signalScore < 4;
    if (hasStaleBeyer || tooFewSignals) {
      conviction = 'MEDIUM'; // Longshot failed audit — stale figure or weak signals
    }
  }
