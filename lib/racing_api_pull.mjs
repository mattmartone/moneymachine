  ...
  for (const meet of meets) {
    try {
      ...
      for (let raceIdx = 0; raceIdx < races.length; raceIdx++) {
        ...
        // Update post time on the race
        if (race.post_time_long || race.post_time) {
          ...
          if (raceRow.rows.length > 0) {
            let postTimeStr = null;
            if (race.post_time_long) {
              const dt = new Date(parseInt(race.post_time_long));
              postTimeStr = dt.toLocaleTimeString('en-US', { hour12: false, timeZone: 'America/New_York' });
            } else if (race.post_time) {
              postTimeStr = race.post_time;
            }
            if (postTimeStr) {
              await pool.query(`UPDATE races SET post_time = $1 WHERE id = $2`, [postTimeStr, raceRow.rows[0].id]);
            }
          }
        }
