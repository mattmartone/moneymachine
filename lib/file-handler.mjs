...
async function parseDRFContent(content) {
  ...
  const trackName = races.size > 0 ? [...races.values()][0].track : 'Unknown';
  return { races: races.size, entries: count, trackName };
}
...
export async function handleSlackFiles(files) {
  ...
  for (const file of files) {
    ...
    if (filename.endsWith('.zip')) {
      ...
      for (const drf of drfFiles) {
        ...
        trackResults.push({ filename, races: result.races, entries: result.entries, trackName: result.trackName });
      }
  ...
  return { totalRaces, totalEntries, trackResults };
}
