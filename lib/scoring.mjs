...
export async function tagCommission(date) {
  ...
  for (const sc of rows) {
    ...
    // Tag strategy activations on all bets for this race
    const betRows = await pool.query(`SELECT id FROM bets WHERE race_id = $1 AND conviction = 'COMMISSION'`, [c.race_id]);
    const strategyMap = [
      { signal: c.s1_fired, id: 6 },   // S1 — Elite Jockey on Bomb
      { signal: c.s4_fired, id: 3 },   // S4 — Hot Barn at a Price
      { signal: c.s5_fired, id: 10 },  // S5 — Distance Stretch-out
      { signal: c.s6_fired, id: 7 },   // S6 — Best Last-Race Beyer
      { signal: c.s9_fired, id: 4 },   // S9 — Earnings Leader
      { signal: c.fave_vulnerable, id: 1 }, // Spot the Vulnerable Favorite
      { signal: true, id: 33 },         // Beyer Ceiling Box (always)
    ];
    const firedStrategies = strategyMap.filter(s => s.signal).map(s => s.id);

    for (const bet of betRows.rows) {
      for (const stratId of firedStrategies) {
        await pool.query(
          `INSERT INTO strategy_activations (bet_id, strategy_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
          [bet.id, stratId]
        ).catch(() => {});
      }
    }
