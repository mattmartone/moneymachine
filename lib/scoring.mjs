...
export async function tagCommission(date) {
  // Try HIGH first, fall back to top MEDIUM if no HIGH exist
  ...
  if (rows.length === 0) {
    // Fall back to top 10 MEDIUM by composite
    const fallback = await pool.query(`
      SELECT id, race_id, composite_score
      FROM scored_candidates
      WHERE date = $1 AND conviction = 'MEDIUM' AND status = 'scored'
      ORDER BY composite_score DESC
      LIMIT 10
    `, [date]);
    rows = fallback.rows;
    if (rows.length > 0) {
      console.log(`[COMMISSION] No HIGH conviction — publishing top ${rows.length} MEDIUM picks`);
    }
  }

  if (rows.length === 0) {
    console.log(`[COMMISSION] No scored candidates for ${date}`);
    return { tagged: 0 };
  }
