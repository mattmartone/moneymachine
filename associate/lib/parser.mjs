...
export async function parseDRFFile(filePath) {
  ...
  // Check for existing — force reload
    ...
    await pool.query(
      `INSERT INTO entries (race_id, horse_id, post_position, morning_line_odds, jockey, trainer, weight, owner, running_style, best_beyer, last_beyer, days_since_last, lifetime_earnings, past_performances, stats)
       ...
       ON CONFLICT (race_id, horse_id) DO UPDATE SET
         past_performances = EXCLUDED.past_performances, stats = EXCLUDED.stats,
         running_style = EXCLUDED.running_style, best_beyer = EXCLUDED.best_beyer,
         last_beyer = EXCLUDED.last_beyer, days_since_last = EXCLUDED.days_since_last`,
      [raceId, horseId, entry.post_position, entry.morning_line_odds, entry.jockey, entry.trainer,
       ...
       entry.days_since_last, null, JSON.stringify(entry.past_performances), JSON.stringify(entry.stats)]
    );
    count++;
  }
